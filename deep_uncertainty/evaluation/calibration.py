...
def display_bayes_credible_interval(
    rv, p: float, ax: Optional[plt.Axes] = None, show=True
) -> Tuple[float, float]:
...
def get_pct_of_targets_in_pred_credible_interval(
    y_true: np.ndarray, posterior_predictive_distribution, p: float = 0.95
) -> float:
    ...
    """
    credible_interval_min, credible_interval_max = get_bayes_credible_interval(
        posterior_predictive_distribution, p
    )
    target_is_in_credible_interval = (y_true >= credible_interval_min) & (
        y_true <= credible_interval_max
    )
...
def plot_regression_calibration_curve(
    ...
    actual_pct_of_targets_in_pred_credible_intervals = [
        get_pct_of_targets_in_pred_credible_interval(y_true, posterior_predictive_distribution, p)
        for p in credible_interval_probabilities
    ]
    ...
    ax.plot(
        expected_pct_of_targets_in_pred_credible_intervals,
        expected_pct_of_targets_in_pred_credible_intervals,
        linestyle="--",
        color="red",
...
def compute_mean_calibration(y_true: np.ndarray, posterior_predictive_distribution) -> float:
    """Given targets and a probabilistic regression model (represented as a random variable over the targets), compute the mean calibration of the model.

    The mean calibration is defined as 1 minus 4/3 the area between the calibration curve of a perfectly calibrated model (y = x)
    ...
    def f(p: float):
        pct_in_pred_credible_interval = get_pct_of_targets_in_pred_credible_interval(
            y_true, posterior_predictive_distribution, p
        )
        return abs(pct_in_pred_credible_interval - p)

    with warnings.catch_warnings():  # Scipy passes back an annoying integration warning that doesn't affect the output.
        warnings.simplefilter("ignore")
        area_between_model_and_perfect_calibration_curve = quad(f, 0, 1)[0]
...
if __name__ == "__main__":
    ...
    plot_regression_calibration_curve(y_true, posterior, num_bins=9)

    # Should be close to 1.
    print(f"Mean Calibration: {compute_mean_calibration(y_true, posterior)}")
