...
class LogGaussianMixtureNN(L.LightningModule):
    ...
    def __init__(self, members: Iterable[LogGaussianNN]):
        super(LogGaussianMixtureNN, self).__init__()
    ...
    @staticmethod
    def from_config(config: EnsembleConfig) -> LogGaussianMixtureNN:
        """Construct a LogGaussianMixtureNN from a config. This is the primary way of building an ensemble.

        Args:
            config (EnsembleConfig): Ensemble config object.

        Returns:
            LogGaussianMixtureNN: The specified ensemble of GaussianNN models.
        """
        checkpoint_paths = config.members
        members = []
        for path in checkpoint_paths:
            member = LogGaussianNN.load_from_checkpoint(path)
            members.append(member)
        return LogGaussianMixtureNN(members=members)
