...
class DiscreteRandomVariable:
    ...
    def __init__(self, dimension: int, max_value: int = 2000, use_torch: bool = False):
    ...
    def pmf(self, x: int | np.ndarray | torch.Tensor) -> float | np.ndarray | torch.Tensor:
        ...
        if not self.use_torch:
            ...
            result = self.pmf_vals[x, torch.arange(d)]
            if result.numel() == 1:
                return result.item()
    ...
    def cdf(self, x: int | np.ndarray | torch.Tensor) -> float | np.ndarray | torch.Tensor:
        ...
        if not self.use_torch:
            ...
            result: torch.Tensor = self.cdf_vals[x, torch.arange(d)]
            if result.numel() == 1:
                return result.item()
        return result
    ...
    def rvs(self, size: int | tuple) -> np.ndarray | torch.Tensor:
        ...
        if not self.use_torch:
            ...
            U = torch.rand(size)
            draws = torch.zeros(U.numel(), self.dimension)
    ...
    def pmf_vals(self) -> np.ndarray | torch.Tensor:
        if self._pmf_vals is None:
            truncated_support = (
                ...
                if not self.use_torch
                else torch.arange(self.max_value).reshape(-1, 1)
            )
