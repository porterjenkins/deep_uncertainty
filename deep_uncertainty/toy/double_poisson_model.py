...
from deep_uncertainty.utils.model_utils import get_mean_preds_and_targets_DPR, train_regression_nn
...
class DoublePoissonNN(nn.Module):
    ...
    def forward(self, x):
        ...
        alpha = torch.exp(self.alpha_layer(x))  # Ensure alpha is positive since it's a scale parameter
...
def double_poisson_loss(y_pred, y_true, x):
    ...
    for i in range(len(beta)): # index
        ...
        term1 = -torch.log((1+ ((1-gamma)/(12 * gamma * lambda_i)) * (1 + 1/(gamma * lambda_i))).clamp(max=10,min=1e-6)) #(32, 1)
        term2 = -y_true[i] # ([1])
